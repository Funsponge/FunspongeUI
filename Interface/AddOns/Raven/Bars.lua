...
--		Nest_SetBarGroupAttribute(bg, "clockEdge", bp.clockEdge) -- save for clock animations, removed in 5.0.4 release
		...
		Nest_SetBarGroupAttribute(bg, "headerGaps", bp.headerGaps and bp.noHeaders) -- convert headers into spaces for tracker bar groups
		local sf = "alpha"
...
local function UpdateBar(bp, vbp, bg, b, icon, timeLeft, duration, count, btype, ttType, ttID, ttUnit, ttCaster, isMine)
	...
	if not (bp.showNoDuration and bp.showOnlyNoDuration) and not ((timeLeft == 0) and (duration == 0)) then -- bar with duration
		if bar and Nest_IsTimer(bar) then -- existing timer bar
			...
			if bar then Nest_StartTimer(bar, timeLeft, duration, maxTime); if b.ghost then bar.attributes.ghostDuration = b.delayTime or 5 end end
		end
	...
	if bar then
		bat = bar.attributes -- optimization to address overhead from setting a large number of bar attributes
...
local function UpdateGhostBars(bp, bg)
	...
	for _, bar in pairs(Nest_GetBars(bg)) do -- first find any elapsed timer bars that might be made into ghosts
		if Nest_IsTimer(bar) then
			local bat = bar.attributes
			if bat.updated then
		...
		for _, bar in pairs(Nest_GetBars(bg)) do
			if Nest_IsTimer(bar) then
				...
				if bat.ghostDuration and not bat.updated and not activeSpells[bat.tooltipSpell or 0] then -- check if candidate for ghost bar
					if not bat.ghostTime then Nest_SetCount(bar, nil); bat.ghostTime = now + bat.ghostDuration end
		...
		for _, bar in pairs(Nest_GetBars(bg)) do
			if Nest_IsTimer(bar) then
				...
				if not bat.updated and not activeSpells[bat.tooltipSpell or 0] then
					if not bat.ghostTime then Nest_SetCount(bar, nil); bat.ghostTime = now + (bp.delayTime or 5) end
					if bat.ghostTime >= now then bat.updated = true end
				end
			end
		end
